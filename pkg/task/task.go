...
type (
	...
	Task           struct {
		...
		*slog.Logger
		...
		startup, shutdown                                                  *m7sutil.Promise
		parent                                                             *Job
...
func (task *Task) reset() {
	...
	task.shutdown = m7sutil.NewPromise(context.Background())
	task.startup = m7sutil.NewPromise(task.Context)
}
